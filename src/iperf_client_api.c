...
iperf_connect(struct iperf_test *test)
{
	...
	test->ctrl_sck = netdial(test->settings->domain, Ptcp, test->proto_num, test->bind_address, 0, test->server_hostname, test->server_port, test->settings->connect_timeout);
    if (test->ctrl_sck < 0) {
        i_errno = IECONNECT;
        return -1;
    }
