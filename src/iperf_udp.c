...
iperf_udp_accept(struct iperf_test *test)
{
     ...
     */
    test->prot_listener = netannounce(test->settings->domain, Pudp, test->proto_num, test->bind_address, test->server_port);
...
iperf_udp_listen(struct iperf_test *test)
{
    ...
    if ((s = netannounce(test->settings->domain, Pudp, test->proto_num, test->bind_address, test->server_port)) < 0) {
        i_errno = IESTREAMLISTEN;
        return -1;
    }
...
iperf_udp_connect(struct iperf_test *test)
{
    ...
    if ((s = netdial(test->settings->domain, Pudp, test->proto_num, test->bind_address, test->bind_port, test->server_hostname, test->server_port, -1)) < 0) {
